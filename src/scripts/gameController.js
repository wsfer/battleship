...
class GameController {
    ...
    constructor(
        ...
        this.#currentPlayerTurn = currentPlayerDOM;
        this.#combatLog = combatLogDOM;
    ...
    async play(coords) {
        ...
        defender.gameboardDOM.classList.add('disabled');
        const attack = attacker.player.attack(defender.player, coords);
        this.#combatLog.textContent = `${attacker.player.name} attacked ${attack.target}`;

        if (attack.isShip) {
            if (!this.#sounds.music.muted) {
                this.#sounds.explosion.play();
            }
            defender.squares[attack.x * 10 + attack.y].appendChild(
                new Range().createContextualFragment(
                    '<div class="discovered-ship"></div>'
                )
            );
        } else {
            if (!this.#sounds.music.muted) {
            ...
            defender.squares[attack.x * 10 + attack.y].appendChild(
                new Range().createContextualFragment(
                    `<svg class="discovered-water" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        ...
        if (attack.gameover) {
            this.#gameOver(attacker.player.name);
            return;
        }

        this.#players.push(attacker, defender);
        attacker.gameboardDOM.classList.remove('disabled');
        this.#currentPlayerTurn.textContent = `${defender.player.name} turn`;
